...
Aura 0.1.6 聚焦对话稳定性、复合 MCP 任务和可诊断性。新安装仍默认使用云端 GLM；MNN / Local Qwen 链路完整保留，只有用户明确选择本地模型且模型已安装时才会预加载。

### 新增

- 增加证据门控的复合任务编排：拆分必须完成的子目标，按目标记录真实工具结果，降低多工具请求漏项和无证据总结。
- 增加 MCP 渐进加载与服务器内工具裁剪，并补充远程 Agent、浏览器边界和长任务状态的基础模型。
- Debug APK 增加诊断导出入口，可导出脱敏日志、应用/设备信息和运行状态；该能力及 FileProvider 不进入 release 包。
- 地图类工具结果增加结构化卡片和系统地图跳转交互。

### 变更

- 统一对话、记忆、提醒和 MCP 顶部工具的图标按钮、弹层标题栏与数量展示。
- 优化流式回复、工具阶段占位、复合结果合并、自动滚动和离底提示，减少结果插入时的跳动。
- MCP 连接检查支持不同服务器并行执行；工具重试限制为可安全重放且符合瞬时错误条件的调用。
- GitHub Release 增加标签/应用版本/更新日志一致性校验，并在打包前执行单元测试和 release lint。

### 修复

- 修复工具调用前后草稿未清理、持久化消息与流式消息重合导致的回复重复显示。
- 修复 Anthropic SSE 多工具索引累积、裸字符串参数兼容、错误结果循环和部分 MCP 工具结果解析问题。
- 修复查询地点后列表频繁上下拉动、历史对话切换和流式交互状态不稳定的问题。

### 验证

- `testDebugUnitTest`：672 个测试通过，0 失败。
- 本地 `lintRelease`：0 errors；release shrinker 与 APK 构建通过。
- 正式 APK 由 tag 工作流使用 GitHub Secrets 中的 release keystore 签名；本地缺少 `keystore.properties` 时生成的 debug 签名产物不用于发布。
