...
### 近期打磨（2026-08-16 · 可用性批次）

> 对应 `docs/chat-ux-benchmark.md` 第 3 批（可用性基线），真机验收通过。

- **输入区**：回车改换行（`ImeAction.Default`，发送只走按钮，多行输入不再丢内容）；一无所获的发送失败自动回填输入文本与图片（用户已开始打下一条时不覆盖）；发送/停止/图片处理三态按钮 `AnimatedContent` 缩放淡入 morph；图片预览卡进出动画。
- **Markdown 链接**：`[label](url)` 渲染为可点击 `LinkAnnotation.Url`（primary 色 + 下划线，http 前缀校验防协议残渣误匹配），流式与完成态共用。
- **身份锚点与时间感**：AI 消息左侧常驻 28dp mini 头像（流式时 SPEAKING 帧、完成 IDLE 帧）；"⋮"操作行旁显示 HH:mm 时间戳；跨天消息间插入"今天/昨天/M月d日"胶囊分隔。
- **空态引导**：新会话空态加 3 条陪伴向建议 prompt（今天有点累想聊聊/记住一件小事/分享好玩的事），点击仅预填输入框。
