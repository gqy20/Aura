...
>
> **2026-08-09 补充**：本文主体保留首次真机审计快照。聊天页中“缺少停止生成/重新生成/失败重试”“失败只显示 Snackbar”“流式时自动抢回底部”等结论已经过期，当前状态见下方复测记录。
...
- GLM 流式回复、记忆写入/召回、Reminder 权限失败与成功通知、高德 MCP、Photo Picker Vision 均完成端到端验证。
- 聊天页已支持停止生成、失败重试、重新生成、编辑后重发、消息/代码复制，以及用户上滑后的“回到最新消息”入口。
- 错误提示已从瞬时 Snackbar 改为持久错误卡，可关闭或重试；部分回复在失败/取消后保留。
- 工具活动会进入顶部状态与消息内进度；原始 HTTP/JSON 异常不再直接暴露给普通用户。
- 调试中修复 Android ICU 正则不兼容、`map tool` 路由漏判与 MCP 中文裸参数非法 JSON。
- 当前主要剩余体验问题：冷启动约 5–7 秒并伴随明显掉帧；停止生成到网络协程完全结束仍可能有数秒延迟。
...
| 5 | Chat | 09~12 | ✅ | Tool 详情、停止/重试/重新生成、编辑后重发、消息复制与离底新消息提示已具备；中文输入仍需人工 IME 验收 |
...
- ~~没有“重新生成”按钮~~ — 2026-08-09 已补：assistant 消息菜单与中断状态均可重新生成
- ~~没有“停止生成”按钮~~ — 2026-08-09 已补：输入栏在生成中切换为停止按钮，并保留部分回复
...
- **改**:
  - ~~Chat 流式响应失败时只在底部 Snackbar 提示~~ — 2026-08-09 已改为持久错误卡 + 重试，部分 assistant 内容保留
...
## 8. 改进项跟进（2026-06-16 更新）

报告生成 16 天后，跟进"最值得做的下一批改进"7 项的落地状态（commit 引用以 `roadmap.md` / `CHANGLOG.md` 为准）：

| # | 改进项 | 状态 | commit / 引用 | 说明 |
|---|--------|------|--------------|------|
| 1 | Memory 卡片长按入口不可见 | ✅ **已修** | `1bad958` (M1 收尾) | `MemoryRoomScreen` 加长按弹层（置顶 / 取消置顶 / 归档 / 取消归档 / 删除） |
| 2 | Onboarding 第 1 步空文本不让进 | ✅ **已修** | `9fa58ab` (M3 PoC) | 改为"5/5 必填，1/5 可空"；行为与代码注释一致 |
| 3 | Chat 中文 IME 不可用 | ⏳ **ADB 限制，非真 bug** | — | 报告已注明"adb 自身限制，非 app bug"；真机 Gboard 拼音可正常进 |
| 4 | Home 卡片长按"隐藏依据"无撤销入口 | 🚧 **仍未做** | — | `mutedUntil` 字段已存（`InsightEntity`），但 Settings 缺"已静音的 Insight"列表 |
| 5 | Insight "和 Aura 聊聊" 预填 prompt 顶栏无视觉提示 | 🚧 **仍未做** | — | prefill 路由已通（`ba6ebad`），但 Chat 顶栏没视觉标记 |
| 6 | MCP 自定义 URL 无响应式校验 | 🚧 **仍未做** | — | LLM 连通性检查已加（`1bad958`），但 MCP 端到端校验未做 |
| 7 | Onboarding 无"重新走一遍"入口 | 🚧 **仍未做** | — | DataStore `onboarding_completed_at` 已存，缺重置入口 |

### 优先级建议的落地率

报告原文 11 项优先级建议的落地情况：

- **P0（3 项）**：2 项已修（#1, #2），0 项未做，1 项 ADB 限制跳过。**落地率 67%**。
- **P1（3 项）**：0 项已修，3 项仍未做（#6 部分已做：LLM 连通性检查）。**落地率 0%**。
- **P2（3 项）**：0 项已修，3 项仍未做。**落地率 0%**。
- **P3（2 项）**：0 项已修，2 项仍未做。**落地率 0%**。

### 整体判断

报告聚焦的"高优功能可见性"（P0 3 项）已落地 2 项，剩余 1 项为 ADB 限制跳过 — P0 阶段基本闭环。

P1-P3 集中在"长期可用性 + 错误恢复 + 多 Insight 区分"等体验增强，依赖 M5 Pulse 周期任务和 M6 产品化加固推进。当前优先级被 M3/M4 主线（M2 Insight + M3 端到端 + M4 vision）取代，文档归档后可作为 M6+ 排期参考。

### 后续建议

- **P1 #6（MCP 响应式校验）**：可独立做，工时 0.5d，对 MCP 用户体验改善明显，建议提前排期。
- **P1 #7（Settings 加"已静音的 Insight"列表）**：与 `mutedUntil` 字段已对接，工时 1d，Insight 闭环体验补完。
- **P2 #8（Onboarding 重新走一遍）**：与 M5 Onboarding 复盘结合做，0.5d。
- **其他 P1-P3 项**：并入 M6 产品化加固统一排期。
